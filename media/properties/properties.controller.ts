...
import { UploadPropertyImageDTO, DeletePropertyImageDTO } from '../dtos/media.dto';
import { AccessDeniedError } from '../errors/media.errors';

export default class PropertiesController extends Controller implements IControllerMethods {
    ...
    /**
     * POST /media/properties - Get presigned upload URL + durable CDN URL for a property image.
    ...
    async post(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
        ...
        try {
            const dto = new UploadPropertyImageDTO();
            const { file_name, mime_type } = dto.validate(JSON.parse(event.body || '{}'));

            const s3Key = `cdn/${organisation.uuid}/properties/${uuidv4()}-${file_name}`;
    ...
    /**
     * DELETE /media/properties - Delete a property image object from S3.
    ...
    async delete(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
        ...
        try {
            const dto = new DeletePropertyImageDTO();
            const { s3_key } = dto.validate(JSON.parse(event.body || '{}'));

            const expectedPrefix = `cdn/${organisation.uuid}/properties/`;
            ...
            return {
                statusCode: 200,
                body: JSON.stringify({ message: 'Property image deleted successfully', s3_key }),
            };
        } catch (error) {
            return this.handleError(error);
        }
