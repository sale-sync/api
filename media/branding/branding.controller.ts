...
import { UploadBrandingImageDTO, DeleteBrandingImageDTO } from '../dtos/media.dto';
import { AccessDeniedError } from '../errors/media.errors';

export default class BrandingController extends Controller implements IControllerMethods {
    ...
    /**
     * POST /media/branding - Get presigned upload URL + durable CDN URL for a website branding logo.
     * Stored separately from the Folder/Media library and from the org's own account logo
     * (/media/organisation): no DynamoDB record is created here, just an S3 object under its own prefix.
     * Restricted to owner/admin (website branding is organisation-editing).
     *
    ...
    async post(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
        ...
        try {
            ...
            const dto = new UploadBrandingImageDTO();
            const { file_name, mime_type } = dto.validate(JSON.parse(event.body || '{}'));

            const s3Key = `cdn/${organisation.uuid}/branding/${uuidv4()}-${file_name}`;
    ...
    /**
     * DELETE /media/branding - Delete a branding logo object from S3.
    ...
    async delete(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
        ...
        try {
            ...
            const dto = new DeleteBrandingImageDTO();
            const { s3_key } = dto.validate(JSON.parse(event.body || '{}'));

            const expectedPrefix = `cdn/${organisation.uuid}/branding/`;
            ...
            return {
                statusCode: 200,
                body: JSON.stringify({ message: 'Branding image deleted successfully', s3_key }),
            };
        } catch (error) {
            return this.handleError(error);
        }
