...
import { TestimonialsService, NoDraftToPublishError } from '../services/testimonials.service';
import { UpdateTestimonialsDraftDTO } from './testimonials.dto';

class TestimonialsController extends Controller implements IControllerMethods {
    ...
    // GET /organisations/testimonials — no role gate, any org member can view the current record.
    async get(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
        ...
        const record = await this.testimonialsService.get(organisation.uuid);
    ...
    // PATCH /organisations/testimonials — update the draft stats/testimonials list. Restricted to owner/admin.
    async patch(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
        ...
        try {
            ...
            const body = new UpdateTestimonialsDraftDTO().validate(JSON.parse(event.body || '{}'));
            const updated = await this.testimonialsService.updateDraft(organisation.uuid, body);
    ...
    // POST /organisations/testimonials — publish (draft -> data). Restricted to owner/admin.
    async post(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
        ...
        try {
            ...
            const updated = await this.testimonialsService.publish(organisation.uuid);
