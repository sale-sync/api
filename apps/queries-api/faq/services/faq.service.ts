...
import type { FaqRecord } from '@sale-sync/shared/src/types';
...
// Read-only, same `data`/`draft` singleton shape as ArticlesService/LocationsService — this stack
// never writes FAQ content (see queries.template.yaml's DynamoDBReadPolicy); writes go through
// api/apps/api/organisation/faq/. Callers must only ever read `.data` (published), never `.draft`.
