...
import type { TermsAndConditionsRecord } from '@sale-sync/shared/src/types';
...
// Read-only mirror of api/organisation/services/terms-and-conditions.service.ts's get() — this
// stack never writes terms-and-conditions content (see queries.template.yaml's DynamoDBReadPolicy),
// so put/updateDraft/publish are deliberately not carried over. Callers must only ever read
// `.data` (published), never `.draft`.
