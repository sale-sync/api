...
class ByIdController extends Controller implements IControllerMethods {
    ...
    // GET /payments/by-id?id={plan_id slug}
    async get(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
        ...
        if (!plan) {
            return { statusCode: 404, body: JSON.stringify({ message: `Plan '${id}' not found` }) };
        }

        return { statusCode: 200, body: JSON.stringify(plan) };
    }

    // DELETE /payments/by-id?id={plan_id slug}
    async delete(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
        ...
        return { statusCode: 200, body: JSON.stringify({ message: `Plan '${id}' deleted` }) };
    }
}
