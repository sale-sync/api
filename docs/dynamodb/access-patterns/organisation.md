...
| `PK` | `main table`                         | `ORG#ID#${org_id}`           | `Lookup organisation by id. Mostly checking org_id already exist since org_id unique string. data = JSON.stringify({ uuid })` |
| `SK` | `main table`                         | `META`                       | `Fixed sort key for organisation ID lookup records`                                          |
| `PK` | `main table`<br>`inverted-index`     | `USER#{user_email}`          | `Lookup the user by email. data = JSON.stringify({ user_id })`                                                                   |
...
| `Lookup organisation by id`                                  | `PK = ORG#ID#${org_id}`<br>`SK = META`<br>`Item.data → { uuid }`                                                                                                                                                                                                                                         | `GetCommand`    | `main table`     |
| `Lookup user by email`                                       | `PK = USER#${user_email}`<br>`SK = META`<br>`Item.data → { user_id }`                                                                                                                                                                                                                                       | `GetCommand`    | `main table`     |
