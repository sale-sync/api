...
| `SK` | `main table` | `CATEGORY#{business_category}#TEMPLATE#{template_uuid}` | Scopes templates by category; supports list-by-category and point lookup     |
...
| ------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------ | ------------ |
| Create template<br><br>_`POST /templates` — intended for admin tooling; temporarily reachable by any authenticated user until that exists_ | `PK = TEMPLATE`<br>`SK = CATEGORY#{business_category}#TEMPLATE#{template_uuid}`                                                                                                                                                                                                             | `PutCommand`                                     | `main table` |
| List templates by business category<br><br>_Used during onboarding_                  | `PK = TEMPLATE`<br>`begins_with(SK, "CATEGORY#{business_category}#TEMPLATE#")`                                                                                                                                                                                                              | `QueryCommand`                                   | `main table` |
| Get template by ID                                                                   | `PK = TEMPLATE`<br>`SK = CATEGORY#{business_category}#TEMPLATE#{template_uuid}`<br><br>Note: `business_category` is always known from the org metadata, so no GSI is needed                                                                                                                 | `GetCommand`                                     | `main table` |
...
| Get active template<br><br>_Resolve the org's current active template_               | `1. Read template_id from org metadata`<br><br>`PK = ORG`, `SK = META#{org_uuid}`<br><br>-----<br><br>`2. Fetch template from catalogue`<br><br>`PK = TEMPLATE`, `SK = CATEGORY#{business_category}#TEMPLATE#{template_id}`                                                                 | `GetCommand`<br><br>-----<br><br>`GetCommand`    | `main table` |
...
| `PK`                   | `SK`                                          | Attributes                                          |
| ---------------------- | --------------------------------------------- | --------------------------------------------------- |
| `TEMPLATE`             | `CATEGORY#fitness#TEMPLATE#tmpl_uuid_1`            | `data: Template`                                    |
| `TEMPLATE`             | `CATEGORY#fitness#TEMPLATE#tmpl_uuid_2`            | `data: Template`                                    |
| `TEMPLATE`             | `CATEGORY#real-estate#TEMPLATE#tmpl_uuid_3`        | `data: Template`                                    |
...
- **Templates are system data** — intended to be managed via admin tooling. `POST /templates` exists in the API as an interim path for adding to the catalogue until that tooling is built; it should be locked down to admins once it exists.
...
- **`org_uuid` is resolved from the `Organisation` cookie, not a client-supplied parameter** — the org-scoped patterns above (add/list/remove template, set active template, get/update theme) never take `org_uuid` from the request. `organisation/templates/templates.controller.ts` and `organisation/templates/theme/theme.controller.ts` decode it from the `Organisation` cookie JWT (`getOrganisation()` in `@sale-sync/shared`) and return a 404 (`NO_ORGANISATION`) if it's missing or invalid — the same pattern `media.md`'s API and `properties.md` use. `AddTemplateSchema`/`RemoveTemplateSchema`/`SetActiveTemplateSchema`/`UpdateThemeSchema` (`packages/src/dtos/template.ts`) have no `org_uuid` field as a result. `POST /templates` (the global catalogue) and `GET /organisations/templates?category=...` are unaffected — they aren't org-scoped.
